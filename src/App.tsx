...
async function analyzeReferenceImages(images: string[]): Promise<string> {
  const content: GroqContentPart[] = [
    {
      type: "text",
      text: `Analiza el/los personaje(s) en la(s) imagen(es) de referencia adjunta(s) y escribe una descripción visual CONCISA en texto plano (máximo 100 palabras). Para CADA imagen/personaje, indica PRIMERO su género (hombre/mujer) de forma explícita, y luego: complexión física, cabello, rasgos si son visibles, ropa, colores, y si el estilo es de energía brillante/holográfico/neón, el color y tipo de brillo. Si hay varias imágenes, numera los personajes (Personaje 1, Personaje 2...) para que se puedan distinguir. Esta descripción la usará después OTRA IA que NO puede ver imágenes, para mantener a cada personaje visualmente consistente e identificable en muchas ilustraciones. NO uses JSON, NO uses markdown, solo texto plano descriptivo.`
    },
    ...
    try {
      ...
      const holographicSoulInstruction = activeStyle === "HOLOGRAPHIC SOUL"
        ? `ESPECIAL PARA HOLOGRAPHIC SOUL (OBLIGATORIO Y PRIORITARIO SOBRE CUALQUIER OTRA INSTRUCCIÓN DE APARIENCIA):
           ...
           ${visualAnchorImages.length > 0 ? `3. REGLA DE ORO (CONSISTENCIA DE AVATAR — HAY IMÁGENES DE REFERENCIA ADJUNTAS): Queda TOTALMENTE PROHIBIDO especificar atributos físicos concretos del personaje (longitud o color exacto de cabello, prendas de ropa predefinidas, tono de piel específico, rasgos faciales cerrados). NO inventes esos detalles. Toda descripción del personaje en 'imagePrompt' debe referirse de forma abstracta a "the ethereal holographic figure from the reference image", dejando que el avatar real cargado por el usuario defina esos rasgos, sin que el texto del prompt los contradiga o los limite. EXCEPCIÓN OBLIGATORIA: el GÉNERO del personaje SÍ debe especificarse siempre que se pueda determinar de la imagen de referencia — usa "the male ethereal holographic figure"/"the female ethereal holographic figure" (o el nombre propio si la historia le dio uno) en vez del genérico "the ethereal holographic figure" a secas. Si hay más de un personaje (varias imágenes de referencia de personas distintas), CADA escena debe dejar explícito CUÁL de ellos aparece (por género y/o nombre), igual que exige la instrucción general de identificación de personaje.` 
             : `3. REGLA DE ORO (SIN AVATAR DE REFERENCIA CARGADO): Como no hay imagen de referencia en este caso, describe a "an ethereal holographic figure" de forma DELIBERADAMENTE ABSTRACTA: evita fijar longitud/color de cabello, prendas específicas o tono de piel exacto. Mantén solo una silueta humanoide general y coherente entre segmentos (proporciones, altura relativa), sin detalles físicos cerrados que después puedan contradecir un avatar que el usuario cargue más adelante. Si la historia deja claro el género del personaje (ej. "él"/"ella" en el concepto), sí puedes reflejarlo ("the male/female ethereal holographic figure").`}
      ...
      const visualAnchorNote = visualAnchorImages.length > 0
        ? (characterDescriptionFromImages
            ? (activeStyle === "HOLOGRAPHIC SOUL"
                ? `REFERENCIA DE PERSONAJE (obtenida analizando la(s) imagen(es) que subió el usuario): ${characterDescriptionFromImages}\n\nA pesar de tener esta referencia, recuerda la regla de oro de Holographic Soul: en el 'imagePrompt' NO describas rasgos físicos fijos como pelo/ropa/piel exacta — pero SÍ usa el género de cada personaje (identificado en la descripción de arriba) explícitamente, ej. "the male ethereal holographic figure" / "the female ethereal holographic figure", y deja claro cuál de los personajes aparece en cada escena si hay más de uno.`
            ...
            : (activeStyle === "HOLOGRAPHIC SOUL"
                ? `NOTA: El usuario adjuntó ${visualAnchorImages.length} imagen(es) de referencia, pero no se pudieron analizar en este intento. Sigue igualmente la regla de oro de Holographic Soul: NO describas rasgos físicos fijos, pero sí indica el género si el concepto de la historia lo deja claro (ej. "the male/female ethereal holographic figure").`
